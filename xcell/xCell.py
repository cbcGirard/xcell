...
class Simulation:
    ...
    def makeUniformGrid(self, nX, sigma=np.array([1., 1., 1.])):
        ...
        nodeOffsets = np.array(
            [np.dot(util.toBitArray(i), elOffsets) for i in range(8)])
    ...
    def setBoundaryNodes(self, boundaryFun=None, expand=None, sigma=None):
        ...
        if expand is None:
            ...
            xyz = np.array([[x, y, z] for z in [0, nX-1]
                           for y in [0, nX-1] for x in [0, nX-1]], dtype=np.uint64)
    ...
    def __selByDoF(self, dofNdx):
        ...
        if nCur < 0:
            ...
            selector = np.logical_and(
                self.nodeRoleTable == 2, self.nodeRoleVals == nCur)
...
class SimStudy:
    ...
    def plotTimes(self, xCat='Number of elements', sortCat=None):
        ...
        for cat in plotnames:
            visualizers.importAndPlotTimes(
                logfile, onlyCat=sortCat, onlyVal=cat, xCat=xCat)
            plt.title(cat)

    def plotAccuracyCost(self):
        logfile = os.path.join(self.studyPath, 'log.csv')
        visualizers.groupedScatter(
            logfile, xcat='Total time', ycat='Error', groupcat='Mesh type')
