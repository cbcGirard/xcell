...
if args.vids:
    #ani = img.animateStudy('volt-'+args.strat, fps=30.)
    #erAni = err.animateStudy('error-'+args.strat, fps=30.)
    anis=[an.animateStudy(l+args.strat,fps=30.) for an,l in zip(animators, aniNames)] # erAn=[an.animateStudy(fps=30.) for an in animators]
...
if args.post:
    ...
    for fmt in ['.png','.svg','.eps']:
        ...
        for d, l in zip(data, labels):
            # ax[0].semilogy(tv[::nskip], np.abs(d['volErr'])/np.mean(np.abs(d['volAna'])), label=l)
        ...
        study.savePlot(f2, os.path.join(
            folderstem, 'ring%dsummary' % args.nRing), ext=fmt)

        study.savePlot(f, os.path.join(folderstem, 'ring%dcomparison' % args.nRing), ext=fmt)
