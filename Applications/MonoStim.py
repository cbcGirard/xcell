...
study,_=com.makeSynthStudy('NEURON/Stim2/Dual_fixed_y'+str(elecY),xmax=domX,)
...
class ThresholdStudy:
    ...
    def _runTrial(self,amplitude,analytic=False):
        ...
        if self.isBiphasic:
            tstim,vstim=xc.nrnutil.makeBiphasicPulse(amplitude,2., self.pulsedur)
        else:
            tstim,vstim=xc.nrnutil.makeMonophasicPulse(amplitude,2., self.pulsedur)
...
if analytic:
    ...
    ax.set_title(r'Activation threshold for %.1f ms %sphasic pulse %d $\mu m$ away'%(tpulse,stimStr,elecY))

    if save:
        dset={'nElec':nElec,'nTot':nTot,'thresh':threshSet,'threshAna':threshAna}
...
if combineRuns:
    ...
    ax.legend()
    ax.set_title(r'Activation threshold for %.1f ms %sphasic pulse'%(tpulse,stimStr))
