...
if meshtype == 'uniform':
    ...
    setup.make_adaptive_grid(ref_pts=np.zeros((1, 3)),
...
xc.visualizers.patchwork_image(ax,
                                 [arr], colormap, color_norm,
                                 extent=bnd)
...
xc.visualizers.show_3d_edges(ax,
                            setup.mesh.node_coords,
                            setup.edges,
                            setup.conductances)

bnodes = setup.mesh.get_boundary_nodes()
xc.visualizers.show_3d_nodes(ax,
                              setup.mesh.node_coords[bnodes],
                              node_values=np.ones_like(bnodes),
                              colors='r')
